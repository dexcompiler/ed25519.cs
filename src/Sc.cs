// Sc.cs - Scalar Arithmetic mod L for Ed25519
// 
...
public static class Sc
{
    ...
    // These are the constants used in reduction: 666643, 470296, 654183, -997805, 136657, -683901
    ...
    /// <summary>
    /// Reduce a 64-byte input to a 32-byte scalar mod L.
    /// Input: s[0]+256*s[1]+...+256^63*s[63]
    /// Output: s[0]+256*s[1]+...+256^31*s[31] = s mod L
    /// Overwrites s in place (uses first 32 bytes for output).
    /// </summary>
    public static void Reduce(Span<byte> s)
    {
        long s0 = 2097151 & Load3(s);
    ...
    /// All inputs and output are 32 bytes.
    /// </summary>
    public static void MulAdd(Span<byte> s, ReadOnlySpan<byte> a, ReadOnlySpan<byte> b, ReadOnlySpan<byte> c)
    {
        // Load a, b, c into 21-bit limbs
        long a0 = 2097151 & Load3(a);
        ...
        // Schoolbook multiplication: s = a*b + c
        long s0 = c0 + a0 * b0;
        ...
        // Carry and reduce (same algorithm as Reduce)
        long carry0 = (s0 + (1 << 20)) >> 21; s1 += carry0; s0 -= carry0 << 21;
        long carry2 = (s2 + (1 << 20)) >> 21; s3 += carry2; s2 -= carry2 << 21;
        long carry4 = (s4 + (1 << 20)) >> 21; s5 += carry4; s4 -= carry4 << 21;
        long carry6 = (s6 + (1 << 20)) >> 21; s7 += carry6; s6 -= carry6 << 21;
    ...
    /// <summary>
    /// "Clamp" a 32-byte value to make it a valid Ed25519 scalar.
    /// Sets bits to ensure the scalar is in the correct range.
    /// </summary>
    public static void Clamp(Span<byte> k)
    {
        k[0] &= 248;
        k[31] &= 127;
        k[31] |= 64;
